...
def register_mobileclip_runtime(registry: AIRuntimeRegistry) -> None:
    ...
    registry.register('mobileclip', AIRuntimeDescriptor(
        ...
        capabilities=(AIRuntimeCapability.IMAGE_EMBEDDINGS,AIRuntimeCapability.TEXT_EMBEDDINGS,AIRuntimeCapability.ZERO_SHOT_CLASSIFICATION), source_url='https://github.com/apple/ml-mobileclip', code_license='MIT for the official Apple MobileCLIP code', model_license='Apple ML Research Model Terms for MobileCLIP weights', expected_download_size='Approximately 216 MB checkpoint plus Python package/cache overhead', supported_devices=('CPU',), recommended_hardware='CPU-only baseline; Python 3.10 dedicated environment is supported; the official package currently resolves torch>=2.8.0 and torchvision>=0.23.0 for the Product Owner workflow.',
        required_python_packages=(RuntimeDependency('mobileclip','mobileclip @ git+https://github.com/apple/ml-mobileclip.git'),RuntimeDependency('torch','__transitive__'),RuntimeDependency('torchvision','__transitive__'),RuntimeDependency('PIL','Pillow>=10,<11')),
        required_model_files=(RequiredModelFile('mobileclip_s0.pt','Explicitly downloaded apple/MobileCLIP-S0 PyTorch checkpoint; no automatic download in MODEL-002B',size_bytes=216*1024*1024,download_url='https://huggingface.co/apple/MobileCLIP-S0/resolve/main/mobileclip_s0.pt',minimum_size_bytes=200*1024*1024),), provider_factory=lambda cache: MobileCLIPEmbeddingProvider(model_cache_dir=cache), verifier=_verify_mobileclip, python_version_spec='Python 3.10 is the supported dedicated environment for current Product Owner MobileCLIP validation.'
    ))
