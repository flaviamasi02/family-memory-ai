...
from ai_runtime.executor import CommandResult
...
def test_verifying_transitions_to_ready_after_success(tmp_path, monkeypatch):
    manager=create_default_runtime_manager(ApplicationDataPathService(tmp_path,tmp_path))
    record=manager.installation_record('mobileclip'); record.interpreter_path=sys.executable; manager.storage.save_installation(record)
    (Path(record.local_model_cache_path)/'mobileclip_s0.pt').write_bytes(b'checkpoint')
    monkeypatch.setattr(manager.executor, 'imports_available', lambda *args: ())
    observed=[]
    monkeypatch.setattr(manager.executor, 'run_action', lambda *args: CommandResult('verify_provider',0,'{"embedding_dimension": 512}','',0.1))

    result=manager.verify_provider('mobileclip', progress=lambda *_: observed.append(manager.installation_record('mobileclip').installation_state))

    assert result.returncode == 0
    assert AIRuntimeState.VERIFYING.value in observed
    assert manager.installation_record('mobileclip').installation_state == AIRuntimeState.READY.value


def test_verifying_transitions_to_failure_with_diagnostic(tmp_path, monkeypatch):
    manager=create_default_runtime_manager(ApplicationDataPathService(tmp_path,tmp_path))
    record=manager.installation_record('mobileclip'); record.interpreter_path=sys.executable; manager.storage.save_installation(record)
    (Path(record.local_model_cache_path)/'mobileclip_s0.pt').write_bytes(b'checkpoint')
    monkeypatch.setattr(manager.executor, 'imports_available', lambda *args: ())
    monkeypatch.setattr(manager.executor, 'run_action', lambda *args: CommandResult('verify_provider',1,'','model load failed',0.1))

    result=manager.verify_provider('mobileclip')
    record=manager.installation_record('mobileclip')

    assert result.returncode == 1
    assert record.installation_state == AIRuntimeState.FAILED.value
    assert record.last_error == 'model load failed'


def test_cached_success_repairs_stale_verifying_state(tmp_path):
    manager=create_default_runtime_manager(ApplicationDataPathService(tmp_path,tmp_path))
    record=manager.installation_record('mobileclip')
    record.interpreter_path=sys.executable
    ...
    status=manager.status('mobileclip', deep=False)

    assert status.state == AIRuntimeState.READY.value
    assert manager.installation_record('mobileclip').installation_state == AIRuntimeState.READY.value


def test_startup_does_not_preserve_interrupted_verifying_state(tmp_path):
    manager=create_default_runtime_manager(ApplicationDataPathService(tmp_path,tmp_path))
    record=manager.installation_record('mobileclip')
    record.interpreter_path=sys.executable
    record.installation_state=AIRuntimeState.VERIFYING.value
    ...
    assert status.state == AIRuntimeState.FAILED.value
    assert 'did not complete' in status.last_error


def test_settings_completion_refreshes_shared_ready_state(tmp_path):
    try:
    ...
    record.installation_state=AIRuntimeState.READY.value; record.last_validation_result='provider verification passed'
    manager.storage.save_installation(record)
    (Path(record.local_model_cache_path)/'mobileclip_s0.pt').write_bytes(b'checkpoint')
    page=SettingsPage(runtime_manager=manager)

    page._on_ai_runtime_completed('verify', CommandResult('verify_provider',0,'verified','',0.1))

    assert page.ai_detail_labels['Status'].text() == AIRuntimeState.READY.value
    assert page.runtime_step_label.text() == 'Current step: verify completed'
    page.deleteLater()
...
def test_settings_completion_displays_verification_failure(tmp_path):
    ...
    assert page.ai_detail_labels['Status'].text() == AIRuntimeState.FAILED.value
    assert page.runtime_step_label.text() == 'Current step: verification failed'
    assert 'model load failed' in page.ai_plan_box.toPlainText()
    page.deleteLater()
