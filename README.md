...
Settings includes an **AI Models** section backed by the generic AI Runtime Manager. It can show registered providers such as MobileCLIP, inspect the selected Python interpreter, and generate an explicit installation plan with dependencies, model files, destination, licenses, and warnings. MODEL-002A provides the provider-agnostic runtime foundation; MODEL-002B adds the managed MobileCLIP installation and verification flow. MODEL-002A through MODEL-002E are merged: the generic runtime foundation, managed installation, stricter verification, diagnostics workflow, and AI Models metadata layout fix are complete. MODEL-003 is the next planned implementation for first real MobileCLIP image classification.
...
Verification requires imports, the checkpoint file, model/transforms creation, tokenizer creation, and one synthetic image embedding with finite numeric output before Ready is recorded. Ready is recorded only after full provider verification: imports, checkpoint load, provider/model/transforms construction, tokenizer creation, and finite embedding output. The production classifier has not been replaced by MobileCLIP. Repository cleanup after PR #22 is complete, and future work should proceed toward MODEL-003 while preserving optional/local runtime fallback behavior.
