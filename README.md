...
Current limits: production automatic category replacement is not implemented. MODEL-003D exposes advisory semantic category suggestions in Memory Review, while similar-photo UI, near-duplicate assistance, clustering, and semantic search remain future Product Owner-prioritized work. DATA-001A–D and DEV-007 Developer Diagnostics are implemented and Product Owner validated. Normal folder import now registers or reuses an application-managed SQLite library and incrementally reuses unchanged metadata, thumbnails, and embeddings. DATA-001E–H remain planned; PERF-001 and MODEL-004B follow the approved DATA-001 sequence. Legacy sidecars and purpose-specific caches remain where their migration has not yet been completed.
