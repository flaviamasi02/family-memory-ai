...
- Root-cause analysis of the repeatable `374/422` stall found leaked SQLite connections, not a model batch boundary: `sqlite3.Connection.__exit__` does not close the connection, so per-image cache reads/writes accumulated database and WAL descriptors across imports. `EmbeddingStore` now explicitly closes every connection, enforces terminal input accounting, and logs cache/terminal counts. A 422-item cached regression completes `0 new / 422 reused / 0 failed` with no descriptor growth.
