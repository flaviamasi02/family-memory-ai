...
Implementation prompts generated by ChatGPT must always include:

- Why We Test: why the feature exists, what is being validated, and which regressions are being checked.
- Manual Test Plan: how to manually test the feature, expected behaviour, persistence checks, regression checks, and expected results.
- Acceptance Checklist: Feature works, Existing behaviour preserved, UI regression check, Performance regression check, Documentation updated, Ready for commit.
