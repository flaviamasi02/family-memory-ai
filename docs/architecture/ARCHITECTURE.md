...
Runtime lifecycle state is authoritative persisted metadata, not a value inferred by a UI refresh. Verification explicitly enters `Verifying` and always records one terminal result: `Ready`, `Dependencies missing`, `Checkpoint Missing`, `Failed`, or `Cancelled`. A persisted `Verifying` record without an active operation belongs to an interrupted prior process and is recovered as an interrupted/cancelled, re-verifiable state at startup. Status reads never demote a valid terminal result back to `Verifying`. The application composition shares one `AIRuntimeManager` between Settings, Test Image, and import indexing; lower-level providers continue to accept explicit manager injection and do not use hidden process-global state.
