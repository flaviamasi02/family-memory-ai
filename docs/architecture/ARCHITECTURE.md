...
## DATA-001 — Central Metadata Storage (DATA-001A implemented; Product Owner validation pending)
...
Runtime lifecycle state is authoritative persisted metadata, not a value inferred by a UI refresh. Verification explicitly enters `Verifying` and always records one terminal result: `Ready`, `Dependencies missing`, `Checkpoint Missing`, `Failed`, or `Cancelled`. Application composition performs one idempotent startup recovery decision after constructing Settings with the shared manager. A persisted `Verifying` record belongs to an interrupted process; a persisted `Cancelled` record is truthful for the session that observed cancellation but is not a permanent installation condition. Because older shutdown-generated cancellation records cannot be reliably distinguished from explicit cancellations, either state is moved by `AIRuntimeManager` to recovery-pending `Verifying` on the next application session and verified again. An explicit cancellation remains `Cancelled` during the current session and the Settings Verify action can retry it immediately. Status reads never hard-code `Ready` or erase truthful dependency, checkpoint, execution, and cancellation outcomes. Settings refreshes on verification start and completion. Application composition shares one `AIRuntimeManager` between Settings, Test Image, and import indexing; lower-level providers continue to accept explicit manager injection and do not use hidden process-global state.
