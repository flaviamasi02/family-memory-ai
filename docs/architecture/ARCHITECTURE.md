...
Runtime lifecycle state is authoritative persisted metadata, not a value inferred by a UI refresh. Verification explicitly enters `Verifying` and always records one terminal result: `Ready`, `Dependencies missing`, `Checkpoint Missing`, `Failed`, or `Cancelled`. A persisted `Verifying` record without an active operation belongs to an interrupted prior process: startup keeps it transient and schedules verification again, allowing healthy installations to reach `Ready` and genuine problems to reach their corresponding terminal state. `Cancelled` is reserved for an observed cancellation request. For compatibility with the initial BUG-001 correction, startup recognizes only the legacy `Cancelled` record whose validation marker says `provider verification interrupted`, migrates it back to a recovery-pending `Verifying` state, and re-runs verification; an explicit `provider verification cancelled` record remains terminal but the normal Verify action can retry it. Status reads never demote a valid terminal result back to `Verifying`. The application composition shares one `AIRuntimeManager` between Settings, Test Image, and import indexing; lower-level providers continue to accept explicit manager injection and do not use hidden process-global state.
