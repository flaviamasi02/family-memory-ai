...
Runtime lifecycle state is authoritative persisted metadata, not a value inferred by a UI refresh. Verification explicitly enters `Verifying` and always records one terminal result: `Ready`, `Dependencies missing`, `Checkpoint Missing`, `Failed`, or `Cancelled`. A persisted `Verifying` record without an active operation belongs to an interrupted prior process: startup keeps it transient and schedules verification again, allowing healthy installations to reach `Ready` and genuine problems to reach their corresponding terminal state. It is never relabeled `Cancelled`; that state is reserved for an observed cancellation request. Status reads never demote a valid terminal result back to `Verifying`. The application composition shares one `AIRuntimeManager` between Settings, Test Image, and import indexing; lower-level providers continue to accept explicit manager injection and do not use hidden process-global state.
