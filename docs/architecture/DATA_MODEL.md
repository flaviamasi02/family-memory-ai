...
DATA-001A provides the registry and version-1 foundation. DATA-001B adds the complete version-2 schema and database operations. DATA-001 as a whole and DATA-001E–H remain planned. Its durable boundary is one `family_memory.db` SQLite database per managed library, owned by the application rather than stored beside source photos.
