...
## Historical DATA-001A implementation note (2026-07-28)
...
## Historical DATA-001B implementation note (2026-07-28)
...
Diagnostic usage is explicit and non-destructive: `PYTHONPATH=src python -m storage.diagnostics --help`, then `root`, `list`, `register <chosen-test-root>`, `open <LibraryID>`, `backup <LibraryID> <destination>`, or `validate <LibraryID> <backup>`. Use `--app-data-root <temporary-directory>` to isolate validation. At the DATA-001B checkpoint, registration never occurred implicitly.

Historical DATA-001B boundary (superseded by implemented DATA-001C/D): this increment made **no persistence cutover**. At that checkpoint, normal import was not connected to SQLite; no photos, imports, categories, reviews, embeddings, albums, preferences, face data, JSON, or sidecar content were migrated. Legacy sidecars, JSON profiles, semantic cache, face-foundation database, and in-memory albums remained authoritative, and original photo folders remained untouched. At that checkpoint DATA-001C–H were planned; DATA-001C/D are now implemented and DATA-001E–H remain planned before PERF-001 and MODEL-004B.
...
## Historical PR #46 mixed-folder import regression correction (2026-08-01)
...
## Historical DATA-001C implementation note (2026-08-01)
...
Schema version 4 adds explicit unchanged, added, removed, moved, renamed, and updated counters to `ImportRun`. Version 5 completes the unchanged fast path by persisting the classifier projection required by Photo Browser and Cleanup Review—automatic/effective and relevance categories, album eligibility, confidence, and reason—without migrating category or review history. Rows created before this projection receive one classification pass on their next observation; subsequent unchanged imports reuse the snapshot. Completed scans mark newly absent locations and logical photos missing without physical deletion, while later reappearance restores availability. Developer Diagnostics reports total, active, and removed photo counts plus the last incremental-sync timestamp and summary. Original files, sidecars, MobileCLIP behavior, review decisions, albums, and future DATA-001E–H migration scope are unchanged. Product Owner validation and ChatGPT review were mandatory before PR #47 merge and have completed.


## DATA-001C/D final implementation and validation record (PR #47, 2026-08-01)

Normal folder import now registers or reuses one managed library, opens its application-owned database, and persists stable PhotoIDs, locations, import runs, and run items. Incremental reconciliation performs one supported-media walk and distinguishes unchanged, added, updated, missing, moved, and renamed observations. It preserves stable identities and historical locations, rehydrates capture/classifier state, reuses thumbnails and embeddings for unchanged or conservatively matched relocations, and submits only added/updated eligible photos to new thumbnail and embedding work. Restart and repeat imports reuse persisted identities without duplicate library or photo rows.

The current expected schema is **version 5**: version 3 adds import elapsed time; version 4 adds explicit synchronization counters; version 5 adds the current classifier/relevance snapshot needed for restart-safe Cleanup Review rehydration. These snapshots do not complete category or review-history migration.

`ApplicationServices` owns the authoritative active `MetadataStore`. A worker prepares a library context, but only the current scan completion may publish it on the UI thread. Monotonic scan run IDs, latest-run comparison, pending-folder rejection, and deterministic thread cleanup prevent stale completion from replacing a newer library. Settings reads this shared context, so “No active library” is valid before import and the active LibraryID/database path are visible after successful import. Connections remain per work unit and are not shared across worker/UI threads.
