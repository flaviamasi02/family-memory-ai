...
- Sprint CLEAN-002 (Initial Media Classification Refinement) - Completed
...
## Sprint CLEAN-002 (Refinement) Update

This refinement pass tightened the deterministic initial media classification behavior to reduce false positives for family_photo in ambiguous shared media. The classifier now applies a stricter rule order, broader multilingual filename indicators (including Italian terms), and conservative WhatsApp handling that prefers unknown when strong photo evidence is missing.

Confidence calibration and classification_reason output were also adjusted so confidence is more closely tied to concrete signals such as camera metadata, EXIF date, camera-like filename patterns, and photo-like geometry. Additional tests were added for WhatsApp greeting graphics, Italian screenshot/document/ad patterns, camera-style photo names, and unknown-by-default weak-signal images.
...
- docs/project/PROJECT_STATE.md is the single source of truth for current version, active domain/milestone, and operational implementation status.
