...
The next approved architectural initiative is **DATA-001 — Central Metadata Storage**. Its authoritative specification is `docs/architecture/DATA_001_CENTRAL_METADATA_STORAGE.md`, covering current-state evidence, schema, identity, migration, concurrency, recovery, and DATA-001A–H delivery increments. DATA-001A–B are implemented with automated validation complete and Product Owner validation pending; DATA-001C–H remain planned. **PERF-001 — Semantic Embedding Performance** is planned and not started, and must follow DATA-001. MODEL-004B through MODEL-004F remain planned and not started.
DEV-007 adds the non-destructive Settings-based Product Owner diagnostics surface for DATA-001B; command-line use is optional. DATA-001C remains the next metadata implementation increment, and normal import remains disconnected from SQLite.
...
1. **DATA-001 — Central Metadata Storage** — DATA-001A–B implemented; Product Owner validation pending; DATA-001C–H planned.
...
Root-cause validation of the deterministic `374/422` cache stall found a separate resource leak in `EmbeddingStore`: Python's SQLite connection context manager commits or rolls back but does not close the connection. Every cache lookup and write therefore retained SQLite/WAL file descriptors until garbage collection. The 422-row first import plus the repeated lookup exhausted the process descriptor limit partway through the cached run; the apparent boundary depended on descriptors already held, not model batching. All store operations now use explicit `closing(...)` ownership around transactional connections. A diagnostic 422-item run holds the descriptor count constant and terminates with `processed=0 cached=422 failed=0 cancelled=0`. Immutable embedding run IDs and a two-part terminal/thread-finished lifecycle also ensure that fast cached completion cannot be discarded if GUI delivery follows thread exit.
